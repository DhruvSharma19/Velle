...
const FriendCard = ({ friend }) => {
  ...
  const handleRemove = async () => {
    try {
      await axios.put(
        `https://velle-wtov.onrender.com/api/users/remove/${friend}`,
        {},
  ...
  const fetchUser = async () => {
    try {
      const res = await axios.get(
        `https://velle-wtov.onrender.com/api/users/find/${friend}`
      );
      const res1 = await axios.get(
        `https://velle-wtov.onrender.com/api/post/myPost/${friend}`,
        { headers: { Authorization: "Bearer " + currentUser.jwt } }
      );
      setPosts(res1.data);
      setUser(res.data);
  ...
  const handleMessage = async (e) => {
    ...
    try {
      const res = await axios.post(
        ...
        { senderId: currentUser._id, recieverId: friend },
        { headers: { Authorization: "Bearer " + currentUser.jwt } }
      );
  ...
  return (
    <Container>
      ...
      <button style={{ backgroundColor: "blue" }} onClick={handleMessage}>
        Message
      </button>
    </Container>
  );
