...
const Card = ({ user }) => {
  ...
  const handleReject = async () => {
    try {
      await axios.put(
        `https://velle-wtov.onrender.com/api/users/reject/${user._id}`,
        {},
  ...
  const handleRequest = async () => {
    await axios.put(
      `https://velle-wtov.onrender.com/api/users/request/${user._id}`,
      {},
      { headers: { Authorization: "Bearer " + currentUser.jwt } }
    );
  ...
  const handleAccept = async () => {
    try {
      await axios.put(
        `https://velle-wtov.onrender.com/api/users/accept/${user._id}`,
        {},
        { headers: { Authorization: "Bearer " + currentUser.jwt } }
      );
  ...
  return (
    <Request>
      ...
      <Top>
        <Banner
          src={user.banner ? user.banner : "/images/Background.svg"}
        ></Banner>
      ...
      {currentUser.friends.includes(user._id) ? (
        <Alert1>You Both are friends</Alert1>
      ) : (
        <>
          <Bottom>
